...
$ckpesan=$_GET['kpesan'];
if (!empty($ckpesan)) {
	$ftoken='include/token.php';
	include $ftoken;
	$status="[".sendMessage($userid, $ckpesan, $tbothot)."]";	
//	$status="[".sendMessage("1341792914", $ckpesan, $tbothot)."]";	
					...
					<div class="col-12">
						<div class="card">
							<div class="card-header">
								...
								<h3 class="card-title"> &nbsp History  Saldo Id <?=$userid." <u>".$namauser['nama_seller']."</u> ( ".count($dthistory)." Trx.)" ?>  <small id="loader" style="display: none;" ><i> Prosessing... <i class='fa fa-circle-o-notch fa-spin'></i></i></small></h3>
							</div>
							...
							<tr><td width="40%">
								...
								for ($i = 0; $i > -6; $i--) {
									...
									echo "<option value='?id=history2&id_u=".$_GET['id_u']."&bln=".$mbln."&back=".$_GET['back']."' ".$pil1."> ".$nbln." </option>";
								}
								?>
								</select>
								...
								for ($x=0;$x<count($dtuser);$x++) {
									...
									echo "<option value='?id=history2&id_u=".encrypt($dtuser[$x]['id_user'])."&bln=".$_GET['bln']."&back=".$_GET['back']."' ".$pil." > ".$no.". ".$dtuser[$x]['id_user']." ".$dtuser[$x]['nama_seller']."</option>";
								}
								?>
							</select>
							</td></tr></table>
							...
							<div class="overflow box-bordered" style="max-height: 70vh">
								...
								<table width="100%" style="border:1px solid white;">
								<tr><td align='center' style="padding:5px ;font-family:times;font-weight:bold;font-size:14px;background:black;">TOTAL DEPOSIT BULAN <?=$_GET['bln']?> ID : <?=$userid?> | NAMA : <?=$namauser['nama_seller']?> | <?=rupiah($ttopup)?> | <?=ucwords(terbilang($ttopup))?> Rupiah.</td></tr>
								</table>
						</div>
					</div>
				</div>
