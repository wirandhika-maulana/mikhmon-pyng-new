...
if (!isset($_SESSION["mikhmon"])) {
  header("Location:../admin.php?id=login");
	...
	$cari=$_GET['idsecret'];
//	$_interface=$cari;
	$_loading_interface="Loading";
	$dtvcr 	= $API->comm("/ip/hotspot/active/print",);
	$cudtvcr 	= $API->comm("/ip/hotspot/active/print",["?address" => $cari,]);
	$_interface=$cudtvcr[0]['user'];

}
/*
url: './hotspot/traffic.php?&session='+session+'&iface=<?=$cari?>'
*/
?>
<script>
...
<div class="card">
	<div class="card-header">
		<h3><i class="fa fa-area-chart"></i> <?= $_traffic_monitor ?> <span style="color:yellow;"><?=$cudtvcr[0]['user']?></span>
			<small id="loader" style="display: none;"><i><i class='fa fa-circle-o-notch fa-spin'></i> Processing... </i></small> <?=$_POST['cari']?>
		</h3>
	</div>
	<div class="card-body">
		<div class="row">
			<div class="col-12">
				<form action="" id="interface" method="post">
				<select id="interface" class="dropd pd-4" style="padding:5px;" onchange="location = this.value; loader()">
					<?php
						for ($x=0;$x<count($dtvcr);$x++) {
							$no=1+$x;
							if ($dtvcr[$x]['address']==$cari){
								echo "<option value='./?hotspot=grafik&session=".$session."&idsecret=".$dtvcr[$x]['address']."' selected> ".$no.". ".$dtvcr[$x]['server']."-".$dtvcr[$x]['user']."</option>";
							}else{
								echo "<option value='./?hotspot=grafik&session=".$session."&idsecret=".$dtvcr[$x]['address']."' > ".$no.". ".$dtvcr[$x]['server']."-".$dtvcr[$x]['user']."</option>";
							}
						}
					?>
				</select>
			...
			<script type="text/javascript"> 
				...
				function requestDatta(session,iface) {
					$.ajax({
                        url: './hotspot/traffic.php?session='+session+'&iface=<?=$_GET['idsecret']?>',
						datatype: "json",
				...
				$(document).ready(function() {
					...
					Highcharts.addEvent(
						Highcharts.Series, 'afterInit', function () {
							this.symbolUnicode = {
								circle: '●',
								diamond: '♦',
								square: '■',
							...
							events: {
								load: function () {
									setInterval(function () {
										...
										chart.setTitle({ text: '<?= $_interface ?> ' });
									}, 2500);
								}				
							}
                        },
