				...
				<div class="card-body overflow">
					...
					<?php
						for ($i = 0; $i < $TotalReg; $i++) {
							...
							<i class='fa fa-minus-square text-danger pointer' onclick="if(confirm('Are you sure to remove user active ( <?= $gethotspotactive[$i]['user']; ?> )?')){loadpage('./?remove-user-active=<?= $gethotspotactive[$i]['.id']; ?>&session=<?= $session; ?>')}else{}" title='Remove user <?= $gethotspotactive[$i]['user']; ?>'></i>

							<?php
							
							$dtbind = $API->comm("/ip/hotspot/ip-binding/print",["?mac-address" => "$mac" ]);
							if (empty($dtbind)) {
								echo "&nbsp&nbsp&nbsp <a title='Binding Ip User ".$gethotspotactive[$i]['user']."' href='./?hotspot=binding-ip&idbinding=".$gethotspotactive[$i]['.id']."&session=".$session."'><i class='fa fa-binoculars text-warning pointer' aria-hidden='true'></i></a>";
							}else{
							...
							echo "&nbsp&nbsp&nbsp <a title='Grafik User ".$gethotspotactive[$i]['server']."-".$gethotspotactive[$i]['user']."' href='./?hotspot=grafik&idsecret=".$gethotspotactive[$i]['address']."&session=".$session."'><i class='fa fa-area-chart' aria-hidden='true'></i></a>";
							echo "</td>";
