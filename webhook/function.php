...
function help($data) {
	if ($data=="kirimwa.id") {
		...
		$data .="1. Add Device.\nUntuk menambahkan Device, tinggal di klik saja.\n";
		$data .="   1.1 Status Device.\nUntuk melihat status Device Anda, terkoneksi / tidak dengan Aplikasi.\n";
		$data .="   1.2 Delete Device.\nUntuk menghapus Device Anda dari Aplikasi.\n\n";
		$data .="2. Pairing\nUntuk menyambungkan Aplikasi dengan Device Anda.\n";
		...
		$data .="3. Qouta\nUntuk mmelihat sisa Qouta 100 pengiriman / hari.\nJam reset dilakukan Server setiap jam 07:00 pagi setiap harinya.\n\n";	
		$data .="4. WebHook On\nUntuk mengaktifkan System AutoRespon System Pembuatan Voucher.\n";	
		$data .="   4.1 WebHook Off\nUntuk menonaktifkan  System AutoRespon System Pembuatan Voucher.\n";	
		$data .="   4.2 WebHook Status\nUntuk melihat Status System AutoRespon System Pembuatan Voucher.\n\n";	
		$data .="5. Help\n\n";	
	}else if ($data=="mpwa") {
		$data = "PETUNUK PENGGUNAAN\n";
		$data .= "Untuk Api Mpwa.\n\n";
...
function kirimwa1($tujuan, $pesan) {
	...
	$jid = $tujuan;
	$message = $pesan;
    $imageUrl = "https://mikhmon.mimoassist.homes/img/logobot1.png";
...
function linka($user,$kvcr,$dns) {
	$data="http://".$dns."/login?username=".$user."&password=".$kvcr;
	return $data;
}
...
function belivoucher($id, $usernamepelanggan, $princevoc,$markup, $username, $password, $uptime, $keterangan, $id_own) {
	global $mikbotamdata;
	$data = $mikbotamdata->get('re_settings', ['saldo','id_user'], ['id_user' => $id]);
	$saldoawal = $data["saldo"];
	if ($id==$id_own){
