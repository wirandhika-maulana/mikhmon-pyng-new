...
if (!isset($_SESSION["mikhmon"])) {
  header("Location:../admin.php?id=login");
	...
	$dtvcr 	= $API->comm("/ppp/active/print",);
}
?>
<script>
...
<div class="card">
	<div class="card-header">
		<h3>
			<i class="fa fa-area-chart"></i> <?= $_traffic_monitor ?> <span style="color:yellow;"><?=ucwords($idpell)?></span>
	...
	<div class="card-body">
		<div class="row">
			<div class="col-12">
				...
				<select id="interface" class="dropd pd-4" style="padding:5px;" onchange="location = this.value; loader()"><br>
					<?php
						for ($x=0;$x<count($dtvcr);$x++) {
							$no=1+$x;
							if ($dtvcr[$x]['service']."-".$dtvcr[$x]['name']==$cari){
								echo "<option value='./?hotspot=grafik&session=".$session."&idsecret=".$dtvcr[$x]['service']."-".$dtvcr[$x]['name']."' selected> ".$no.". ".ucwords($dtvcr[$x]['service'])."-".$dtvcr[$x]['name']."</option>";
							}else{
								echo "<option value='./?hotspot=grafik&session=".$session."&idsecret=".$dtvcr[$x]['service']."-".$dtvcr[$x]['name']."' > ".$no.". ".ucwords($dtvcr[$x]['service'])."-".$dtvcr[$x]['name']."</option>";
							}
						}
					?>
				</select>
			...
			<script type="text/javascript"> 
				...
				function requestDatta(session,iface) {
					$.ajax({
					url: './ppp/traffic.php?&session='+session+'&iface=<?=$_GET['idsecret']?>',
						datatype: "json",
							...
							events: {
								load: function () {
									setInterval(function () {
										...
										chart.setTitle({ text: '<?= ucwords($_interface) ?> ' });
									}, 2000);
								}				
							}
                        },
